...
However, simply running RDF2Graph against \gls{wdqs}, the public \gls{sparql} endpoint for Wikidata,
...
so that after creating a \filename{\variable{example}.entities.sparql} file
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \begin{scope}[every path/.style=arrow, midway]
      ...
      \path (data.south) -- node {WDQS} (JSON.north);
...
specifies the Wikidata Query Service \gls{sparql} endpoint for this option,
...
% TODO this is way too long as a chapter introduction.
% split into sections, then write a separate introduction paragraph
