...
some are motivated by the Wikidata use case but still generally useful (\cref{sec:RDF2Graph+Wikidata:cyclic-graphs,sec:RDF2Graph+Wikidata:schema-reduction,sec:RDF2Graph+Wikidata:depth-limit}),
...
some of them may also be merged into the upstream RDF2Graph and RDFSimpleCon repositories in the future.
...
given a query which selects the exemplary items,
downloads the related data for them and runs RDF2Graph against it.
The process is outlined in \cref{fig:process}
and controlled by a Makefile in the RDF2Graph repository,
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    % TODO more details for RDF2Graph (first “normal stuff”, then simplification)
    ...
    \begin{scope}[every path/.style=arrow, midway]
      \path (entities.south) -- node {\command{sed}} (data.north);
      \path (data.south) -- node {\gls{wdqs}} (JSON.north);
      \path (JSON.south) -- node {\command{jq}} (N-Triples.north);
      \path (N-Triples.south) -- node {Fuseki} (Fuseki.north);
      \path (Fuseki.south) -- node {RDF2Graph} (RDF2Graph.north);
      \path (RDF2Graph.south) -- node {\gls{shex} exporter} (ShEx);
      \path (ShEx.south) -- node {\command{pygmentize}} (HTML);
    \end{scope}
  \end{tikzpicture}
...
First, the query selecting the exemplary items is transformed
...
and all \PL{P31}{instance of} statements of items which occur as values of the indirect statements.
This query is run against \gls{wdqs},
which returns the results in JSON format; % TODO glossary for JSON?
they are then piped into a script for the \command{jq} tool (“JSON query”),
which transforms them into N-Triples format, % TODO explain N-Triples?
...
and the RDF2Graph \gls{shex} exporter is run to produce a \gls{shex} file.
...
RDF2Graph was changed to use the predicates \PName{wdt:P31} and \PName{wdt:P279}
...
Therefore, step 2 in the simplification process was adapted % TODO awkward passive voice
...
This was implemented in the final step of the \gls{shex} exporter, % TODO mention shexbuilder.js file name?
...
the Wikidata Shape Expressions Inference Tool,
which makes the Wikidata-aware % “Wikidata-aware”?
version of RDF2Graph easily available to Wikidata editors.
...
The goal of the tool is to make the schema inference process available to other Wikidata editors,
which is necessary because RDF2Graph, its \gls{shex} exporter, and the Makefile-based process around them
...
run RDF2Graph and the \gls{shex} exporter,
...
the versions of various software used by RDF2Graph and the rest of the inference process
...
RDF2Graph required Java 8,
but only Java 7 is installed on Toolforge.
Fortunately, RDF2Graph only used Java 8 for syntactic constructs (“lambda” syntax)
...
but fortunately RDF2Graph is compatible with their older versions which support Java 7,
