...
and \gls{simplification} step 4 (see \cref{sec:Background:RDF2Graph}) was disabled
...
including \gls{RDF2Graph}’s \gls{simplification} step.
...
which are used in the \gls{simplification} step.
...
so queries operating on that graph as part of the \gls{simplification} step
...
it cannot merge them during the \gls{simplification} step.
...
\Gls{RDF2Graph}’s \gls{simplification} step would originally merge all classes into their superclasses,
...
Therefore, step 2 in the \gls{simplification} process was adapted
...
\gls{RDF2Graph} has an optional \gls{simplification} feature
...
several algorithms in the \gls{simplification} step were adjusted to be able to cope with cyclic graphs.
...
will no longer impede \gls{simplification} in other, more concrete classes,
where \gls{simplification} is much more useful anyways.
Without these improvements,
if any part of the \gls{simplification} step failed
(typically with a \lstinline[language=java]{StackOverflowError}),
all \gls{simplification} results would be lost
...
Two steps of the \gls{simplification} process
...
at least if \gls{simplification} was applied to the \gls{schema}.
(Without \gls{simplification}, some \glspl{type link} may be redundant
...
Full installation instructions may be found in the \filename{README.md} file
